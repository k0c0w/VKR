import { authApi, isSignInArgsValidationProblem, setUser } from "@features/authorization";
...
export default function LoginWidget({afterLoginSubmit}:LoginWidgetProps) {
  ...
  useEffect(() => {
    ...
    if (isError && error) {
      ...
      setErrors({
        globalErrorMessage: 'Непредвиденная ошмбка на сервере. Пожалуйста, попробуйте позже.',
        login: undefined,
        password: undefined,
      });
    }
