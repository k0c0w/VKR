...
import { setRoomsOnCurrentLevel } from "./planEditorSlice";
...
function computeProjectiveTransform(cornerPixels: { x: number; y: number }[], imgWidth: number, imgHeight: number)
  ...
  return (x: number, y: number) => {
    ...
    if (denom === 0) {
      return { x: 0, y: 0 }; // Fallback to avoid division by zero
    }
  ...
  function completeLayout(potentialRooms: { text?: string; bbox: number[]; mask: string }[]) {
    ...
    const rooms: Room[] = potentialRooms.flatMap(room => {
      ...
      return contours.map(contour => {
        const coordinates: [number, number][] = contour.map(point => {
          const [x, y] = point; 
          const imgX = x + minX;
          const imgY = y + minY;
        ...
        const polygonIsClosed = coordinates.length > 0 && (coordinates[0][0] !== coordinates[coordinates.length - 1][0] || coordinates[0][1] !== coordinates[coordinates.length - 1][1]);
        if (polygonIsClosed) {
        ...
        /* todo: apply simplification for complex masks
        if (polygon.coordinates[0].length > 15) {
          polygon = simplifyPolygon(polygon, GEOJSON_PRECISION);
        }
        */
    ...
    dispatch(setRoomsOnCurrentLevel(rooms));
  }
