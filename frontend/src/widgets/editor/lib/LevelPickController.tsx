...
import { addLevel as addLevelToState, editLevel, removeLevelAtIndex } from "./planEditorSlice";
...
function EditLevelDialog({open, handleClose, setLevelName}: {open: boolean; handleClose: () => void; setLevelName: (name: string) => void;}) {
  ...
  const { building } = useAppSelector(state => state.planEditorSlice);
...
export default function LevelPickController({building, readonlyMode = false}: {readonlyMode?: boolean; building: Building;}) {
    ...
    const { map, setCurrentLevelIndex, currentLevelIndex } = useBuildingMap();
    const dispatch = useAppDispatch();

    function addLevel(levelName: string) {
      dispatch(addLevelToState({name: levelName}));
    }

    function renameLevel(levelName: string) {
        dispatch(editLevel({levelIndex:currentLevelIndex, levelName}));
    }
    ...
    useEffect(() => {
      map.levelControl?.setRubbishBinDisabled(building.properties.levels.length <= 1);
    }, [map, readonlyMode, building])

    useEffect(() => {
        const onAddLevelClicked = () => setLevelNameDialogOpen(true);
        const switchLevel = ({levelIndex}: {levelIndex: number;}) => setCurrentLevelIndex(levelIndex);
    ...
    }, [map, setCurrentLevelIndex, dispatch, setLevelNameDialogOpen, setRemoveLevelDialogIsOpen, setEditLevelDialogIsOpen]);
    ...
    return <>
        ...
        {!readonlyMode && <RemoveLevelDialog 
            ...
            onRemoveSubmit={() => dispatch(removeLevelAtIndex(currentLevelIndex))}
        />}
