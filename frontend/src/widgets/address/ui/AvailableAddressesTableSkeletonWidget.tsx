...
interface AddressTableSkeletonProps {
  maxWidth?: number | string; // Allow number (px) or string (e.g., '650px')
}
export default function AvailableAddressesTableSkeletonWidget({maxWidth}: AddressTableSkeletonProps) {
    const rows = Array.from({ length: 5 });
    
    return <Box sx={{ maxWidth, mx: 'auto', my: 4 }}>
        {/* Header skeleton */}
