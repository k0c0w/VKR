    ...
    : EKsuClientBase(clientFactory, logger ?? NullLogger<IItEquipmentCatalogue>.Instance), IItEquipmentCatalogue
{
    public Task<Result<ItEquipmentDescription[], ErrorMessage>> GetAllItEquipmentByRoomIdsAsync(long[] roomIds, CancellationToken ct = default)
    {
        if (roomIds.Length == 0)
        {
            return Task.FromResult(Result.Ok<ItEquipmentDescription[], ErrorMessage>([]));
        }

        return roomIds.Length < 3 
    ...
    private Result<List<ItEquipmentDescription>, ErrorMessage> ParseResponse(IDocument html, long catalogueRoomId)
    {
        ...
        const string eKsuController = "e-ksu/";
        try
        {
            ...
            foreach (var row in rows)
            {
                ...
                if (!string.IsNullOrEmpty(onClickAttr))
                {
                    ...
                    if (match.Success)
                    {
                        relativeHistoryUrl = match.Groups[1].Value;
                    }
                    else
                    {
                ...
                equipmentList.Add(new ItEquipmentDescription
                {
                    Id = inventoryNumber,
                    Name = equipmentName,
                    ItEquipmentCardUrl = ToAbsoluteUrl(relativeCardUrl.Replace(jqueryVariablePrefix, eKsuController)).ToString(),
                    ItEquipmentHistoryUrl = ToAbsoluteUrl(relativeHistoryUrl?.Replace(jqueryVariablePrefix, eKsuController) ?? "").ToString(),
    ...
    private FormUrlEncodedContent ConstructRoomItEquipmentListFilterForm(long locatedAtRoomId)
    {
        ...
        var entryPage = context?.User.Claims.FirstOrDefault(x => x.Type == nameof(AuthorizationCredentials.Entry))
                            ?.Value ??
                        string.Empty;
        var thisUserSession =
            context?.User.Claims.FirstOrDefault(x => x.Type == nameof(AuthorizationCredentials.Session))?.Value ??
            string.Empty;
        var sessionHash = context?.User.Claims.FirstOrDefault(x => x.Type == nameof(AuthorizationCredentials.Hash))
                              ?.Value ??
                          string.Empty;
